...
public:
  ...
  RtpInstance(int sockfd, uint8_t rtpChannel)
      : mRtpType(RTP_OVER_TCP), mSockfd(sockfd), mLocalPort(0), mIsAlive(false),
        mSessionId(0), mRtpChannel(rtpChannel) {}
...
public:
  static RtcpInstance *createNew(int localSockfd, uint16_t localPort,
    ...
    //        return New<RtcpInstance>::allocate(localSockfd, localPort, destIp,
    //        destPort);
  }
...
public:
  RtcpInstance(int localSockfd, uint16_t localPort, std::string destIp,
               uint16_t destPort)
      : mLocalSockfd(localSockfd), mLocalPort(localPort),
        mDestAddr(destIp, destPort), mIsAlive(false), mSessionId(0) {}
