...
public:
  static RtspServer *createNew(UsageEnvironment *env,
                               MediaSessionManager *sessMgr, Ipv4Address &addr);

  RtspServer(UsageEnvironment *env, MediaSessionManager *sessMgr,
             Ipv4Address &addr);
  ~RtspServer();
...
public:
  ...
  void start();
  UsageEnvironment *env() const { return mEnv; }
...
private:
  ...
  std::map<int, RtspConnection *>
      mConnMap;                     // <clientFd,conn> 维护所有被创建的连接
