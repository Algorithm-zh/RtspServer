...
H264FileMediaSource *H264FileMediaSource::createNew(UsageEnvironment *env,
                                                    const std::string &file) {
  return new H264FileMediaSource(env, file);
  //    return New<H264FileMediaSource>::allocate(env, file);
}

H264FileMediaSource::H264FileMediaSource(UsageEnvironment *env,
                                         const std::string &file)
    : MediaSource(env) {
  mSourceName = file;
...
H264FileMediaSource::~H264FileMediaSource() { fclose(mFile); }
...
void H264FileMediaSource::handleTask() {
  ...
  while (true) {
    ...
    // LOGI("startCodeNum=%d,naluType=%d,naluSize=%d", startCodeNum, naluType,
    // frame->mSize);
