...
H264FileSink *H264FileSink::createNew(UsageEnvironment *env,
...
H264FileSink::H264FileSink(UsageEnvironment *env, MediaSource *mediaSource)
    : Sink(env, mediaSource, RTP_PAYLOAD_TYPE_H264), mClockRate(90000),
      mFps(mediaSource->getFps()) {
  LOGI("H264FileSink()");
  runEvery(1000 / mFps);
}

H264FileSink::~H264FileSink() { LOGI("~H264FileSink()"); }
...
void H264FileSink::sendFrame(MediaFrame *frame) {
  ...
  if (frame->mSize <= RTP_MAX_PKT_SIZE) {
    ...
    for (i = 0; i < pktNum; i++) {
      ...
      rtpHeader->payload[0] =
          (naluType & 0x60) |
          28; //(naluType & 0x60)表示nalu的重要性，28表示为分片
