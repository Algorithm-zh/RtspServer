...
EventScheduler::EventScheduler(PollerType type) : mQuit(false) {
  ...
  // 创建定时器管理器,一个资源需要一个定时器
  mTimerManager = TimerManager::createNew(
      this); // WIN系统的定时器回调由子线程托管，非WIN系统则通过select网络模型
}
...
// addTimedEventRunAfter 用于在指定的时间间隔后执行一次事件。
// addTimedEventRunAt 用于在指定的绝对时间点执行一次事件。
// addTimedEventRunEvery 用于每隔指定的时间间隔重复执行事件。
Timer::TimerId
EventScheduler::addTimedEventRunAfater(TimerEvent *event,
...
Timer::TimerId
EventScheduler::addTimedEventRunEvery(TimerEvent *event,
...
Poller *EventScheduler::poller() { return mPoller; }
